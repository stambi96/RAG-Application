...
def delete_from_vector_collection(file_name_prefix: str):
    """Deletes all vectors whose IDs start with the given file_name_prefix."""
    collection = get_vector_collection()
    all_ids = collection.get(ids=None)["ids"]
    ids_to_delete = [doc_id for doc_id in all_ids if doc_id.startswith(file_name_prefix)]
    
    if ids_to_delete:
        collection.delete(ids=ids_to_delete)
        st.success(f"🗑️ Deleted {len(ids_to_delete)} chunks for '{file_name_prefix}' from the vector store.")
    else:
        st.warning(f"No vector data found with prefix: {file_name_prefix}")
    ...
    with st.sidebar:
        ...
        if uploaded_files and process:
            for uploaded_file in uploaded_files:
                normalize_uploaded_file_name = re.sub(r"[^\w]", "_", uploaded_file.name)
                with st.spinner(f"Processing {uploaded_file.name}..."):
                    all_splits = process_document(uploaded_file)
                    add_to_vector_collection(all_splits, normalize_uploaded_file_name)

        st.markdown("---")
        st.subheader("🗑️ Delete Stored Vector Data")
        file_to_delete = st.text_input("Enter filename (without extension, e.g. 'Report')")
        delete_button = st.button("❌ Delete Vector Data")

        if delete_button and file_to_delete:
            normalized_name = file_to_delete.translate(
                str.maketrans({"-": "_", ".": "_", " ": "_"})
            )
            delete_from_vector_collection(normalized_name)
